...
def main():
    ...
    GT_folder = osp.join(save_folder, 'gt')
    output_folder = osp.join(save_folder, 'output')
    input_folder = osp.join(save_folder, 'input')
    ...
    for phase, dataset_opt in opt['datasets'].items():
        ...
        for val_data in val_loader:
            ...
            if save_imgs:
                try:
                    tag = (val_data['folder'])
                    print(osp.join(output_folder, tag[0]))
                    cv2.imwrite(osp.join(output_folder, tag[0]), rlt_img)
                    # cv2.imwrite(osp.join(GT_folder, tag[0]), gt_img)
                    #
