...
def read_img_hist(env, path, size=None):
    """read image by cv2 or from lmdb
    return: Numpy float32, HWC, BGR, [0,1]"""
    if env is None:  # img
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED)
...
def read_img_hist_ns_mask(env, path, size=None):
    """read image by cv2 or from lmdb
    return: Numpy float32, HWC, BGR, [0,1]"""
    if env is None:  # img
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED)
        ...
        img = _read_img_lmdb(env, path, size)
    imgs=orirgb2gray(img)
    imgs=np.squeeze(imgs)
    h,w=imgs.shape
    zeros=np.ones([h,w])
    mask=np.where(imgs<=0)
    for i in range(len(mask[0])):
        zeros[mask[0][i],mask[1][i]]=0

    if zeros.ndim == 2:
        zeros = np.expand_dims(zeros, axis=2)
    # some images have 4 channels
    if zeros.shape[2] > 3:
        zeros = zeros[:, :, :3]
    return zeros
...
def read_img_seq_hist_ns_mask(path, size=None):
    """Read a sequence of images from a given folder path
    Args:
        path (list/str): list of image paths/image folder path
    ...
    ns_zero_mask = [read_img_hist_ns_mask(None, v, size) for v in img_path_l]

    ns_zero_masks=np.stack(ns_zero_mask,axis=0)


    ns_zero_masks = torch.from_numpy(np.ascontiguousarray(np.transpose(ns_zero_masks, (0, 3, 1, 2)))).float()

    return ns_zero_masks
...
def read_img_seq_hist(path, size=None):
    """Read a sequence of images from a given folder path
    Args:
        path (list/str): list of image paths/image folder path
    ...
    img_l= [read_img_hist(None, v, size) for v in img_path_l]
